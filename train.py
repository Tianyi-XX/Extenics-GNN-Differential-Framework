...
def evaluate(model, loader, device):
    ...
    with torch.no_grad():
        for data in loader:
            ...
            pred_conflict = (outputs['edge_is_conflict'] > 0.5).float()
            conflict_acc = (pred_conflict == data.edge_is_conflict).float().mean().item()
            severity_mae = F.l1_loss(outputs['edge_severity'], data.edge_severity).item()

            pred_problem = (outputs['edge_is_problem'] > 0.5).float()
            problem_acc = (pred_problem == data.edge_is_problem).float().mean().item()
            ...
            metrics['conflict_acc'].append(conflict_acc)
            metrics['severity_mae'].append(severity_mae)
            metrics['problem_acc'].append(problem_acc)
            ...
            if isinstance(data.domain_name, list):
                for domain in set(data.domain_name):
                    ...
                    domain_metrics[domain]['conflict_acc'].append(conflict_acc)
            else:
                domain_metrics[data.domain_name]['importance_mae'].append(importance_mae)
                domain_metrics[data.domain_name]['conflict_acc'].append(conflict_acc)
...
def run_cross_validation(dataset, relation_mapping, edge_dim, device,
    ...
    for fold, (train_val_idx, test_idx) in enumerate(kf.split(indices)):
        ...
        print(f"  Conflict Acc:     {best_metrics['conflict_acc']:.4f}")
    ...
    metric_keys = ['importance_mae', 'severity_mae', 'val_loss',
                   'conflict_acc', 'relevance_mae', 'problem_acc']
...
def main():
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            ...
            print(f"  Conflict Acc: {val_metrics['conflict_acc']:.4f} | "
    ...
    for domain in sorted(test_domain_metrics.keys()):
        print(f"  {domain}: Conflict Accuracy={test_domain_metrics[domain].get('conflict_acc', 0):.4f}")
